...
test('Browser context Playwright Test', async ({ page }) => 
{
    page.addInitScript(value => {
        window.localStorage.setItem('token', value);
                                                                ...
                                                                // <<<<<<<<<<<<<<FOLLOWING STEPS CAN BE SKIPPED BY USING API>>>>>>>>>>>>>>>>>>>>
    await page.goto('https:rahulshettyacademy.com/client/');    // await page.goto('https:rahulshettyacademy.com/client')
    const productName = "zara coat 3";                          // await page.locator('#userEmail').fill('imhaslop@gmail.com');
    const products = page.locator('.card-body');                // await page.locator('#userPassword').fill('Passwd321@_');
    // const email = userEmail;                                 // await page.locator('[class*="login-btn"]').click();
